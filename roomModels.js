export function roomShapePreviewHTML(shape) {
  ...
  const maxW = Math.max(...sections.map(s => Math.abs(Number(s.x || 0)) + Number(s.w || 0) / 2)) * 2 || Number(shape.w || 14);
  const maxD = Math.max(...sections.map(s => Math.abs(Number(s.z || 0)) + Number(s.d || 0) / 2)) * 2 || Number(shape.d || 14);

  const pieces = sections.map(s => {
    const left = ((Number(s.x || 0) - Number(s.w || 0) / 2 + maxW / 2) / maxW) * 100;
    const top = ((Number(s.z || 0) - Number(s.d || 0) / 2 + maxD / 2) / maxD) * 100;
    const width = (Number(s.w || 0) / maxW) * 100;
    const height = (Number(s.d || 0) / maxD) * 100;
    return `<i style="left:${left.toFixed(2)}%;top:${top.toFixed(2)}%;width:${width.toFixed(2)}%;height:${height.toFixed(2)}%"></i>`;
  }).join('');

  return `<span class="shape-preview habbo-shape-preview real-room-preview">
    <span class="preview-floor">${pieces}</span>
    <b class="preview-wall back"></b>
    <b class="preview-wall left"></b>
    <em class="preview-door"></em>
    <strong class="preview-window"></strong>
  </span>`;
}
...
export function createRoomShell(THREE, block, room, shape) {
  ...
  mergeEdges(northEdges, true).forEach(seg => {
    ...
    const z = seg.key;
    block(length, 4.2, 0.24, room.wall, centerX, 2, z + 0.12);
  });
  ...
  mergeEdges(westEdges, false).forEach(seg => {
    ...
    const x = seg.key;
    block(0.24, 4.2, length, room.wall, x + 0.12, 2, centerZ);
  });

  // Só formatos recortados precisam de parede direita extra.
  const special = ['dividido'];
