...
function animatePlayerWalk(dt = 1/60) {
  ...
  if (isWalking) {
    ...
    p.armL.position.y = 1.18 + Math.abs(opposite) * 0.03;
    p.armR.position.y = 1.18 + Math.abs(step) * 0.03;
    p.legL.position.y = 0.43 + Math.max(0, step) * 0.06;
    p.legR.position.y = 0.43 + Math.max(0, opposite) * 0.06;
  } else {
    ...
    if (p.chest) p.chest.position.y += ((1.28 + breathe) - p.chest.position.y) * 0.12;
    if (p.head) p.head.position.y += ((2.14 + breathe * 0.6) - p.head.position.y) * 0.12;
    p.armL.position.y += (1.18 - p.armL.position.y) * 0.25;
    p.armR.position.y += (1.18 - p.armR.position.y) * 0.25;
    p.legL.position.y += (0.43 - p.legL.position.y) * 0.25;
    p.legR.position.y += (0.43 - p.legR.position.y) * 0.25;
  }
}
