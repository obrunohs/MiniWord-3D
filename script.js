...
import { roomThemes as rooms, roomShapes, roomCategories, roomVisitorOptions, roomTradePreferences } from './rooms.js';
import { createRoomShell, roomShapePreviewHTML } from './roomModels.js';
import { createAvatarModel } from './avatarModels.js';
import { avatarItems, applyAvatarItemIds } from './avatarItems.js';
...
const OWNER_ADMIN_EMAILS = ["scripthc3@gmail.com"];
...
let roomWalkSections = [{ x: 0, z: 0, w: 14, d: 14 }];
...
function keepGameFullscreen() {
  const isTouch = matchMedia('(pointer: coarse)').matches || 'ontouchstart' in window;
  if (!isTouch) return;
  tryMobileLandscapeMode().catch(() => {});
}

document.addEventListener('fullscreenchange', () => {
  if (currentUser && currentRoomData && !document.fullscreenElement) {
    setTimeout(() => keepGameFullscreen(), 350);
  }
});
...
async function login() {
  ...
  try {
    ...
    toggleAdminButton();
    setTimeout(() => toggleAdminButton(), 500);
    setTimeout(() => toggleAdminButton(), 1500);
  } catch (err) {
...
refreshAvatarButton();
  ...
  setTimeout(() => toggleAdminButton(), 100);
  setTimeout(() => toggleAdminButton(), 1000);
  window.adminDebug = adminDebug;
  window.forceShowAdminButtonForDebug = forceShowAdminButtonForDebug;
  window.showAdminNow = () => { toggleAdminButton(); return adminDebug(); };
...
function createRoomRecord({ name, pass = '', description = '', category = 'bate-papo', maxVisitors = 10, tradePreference = 'nao_permitidas', baseRoom = selectedRoom, shape = selectedRoomShape, isFirst = false }) {
  ...
  const record = {
    ...
    description,
    category,
    maxVisitors: Number(maxVisitors || 10),
    tradePreference,
    pass,
    baseRoomId: baseRoom.id,
    shapeId: shape.id,
    createdAt: Date.now(),
    mobis: [ { type: 'janela', x: 0, y: 0, z: -(shape.d / 2) + 0.48, rot: 0, wallMobi: true } ],
    isFirst
  };
  all.push(record);
  saveStoredRooms(all);
...
function ensureInitialRoom(roomName, roomPass) {
  ...
  return createRoomRecord({ name: roomName, pass: roomPass, description: '', category: 'bate-papo', maxVisitors: 10, tradePreference: 'nao_permitidas', baseRoom: selectedRoom, shape: selectedRoomShape, isFirst: true });
}
...
function makeRemotePlayerGroup(profile) {
  const base = characters.find(ch => ch.id === Number(profile.characterId)) || characters[0];
  const c = { ...base, ...(profile.character || {}) };
  const group = createAvatarModel(THREE, block, c, { ...profile, character: c }, 'remotePlayer');
  return group;
}
...
function enterRoom(record) {
  ...
  setTimeout(() => {
    if (scene) {
      ...
      toggleAdminButton();
      setTimeout(() => toggleAdminButton(), 900);
      startRoomRealtime();
    }
    renderRoomsModal();
...
function roomListItem(record, mine = false) {
  ...
  const deleteBtn = mine ? `<button data-delete-room-id="${record.id}" type="button" class="danger-room-btn">Excluir</button>` : '';
  return `<div class="room-list-item"><div><b>${escapeHtml(record.name)}${lock}</b><small>Dono: ${escapeHtml(record.ownerName || record.owner)} • ${shape.name} • ${theme.name} • ${escapeHtml(record.category || 'bate-papo')} • máx. ${Number(record.maxVisitors || 10)}</small></div><div class="room-list-actions"><button data-room-id="${record.id}" type="button">Ir ao quarto</button>${deleteBtn}</div></div>`;
}
...
async function deleteRoom(roomId) {
  ...
  if (!room) return alert('Quarto não encontrado.');
  if (room.owner !== (currentUser.uid || currentUser.login)) return alert('Você só pode excluir seus próprios quartos.');
  if (!confirm(`Excluir o quarto "${room.name}"? Os mobis colocados nele serão recolhidos para o seu inventário.`)) return;

  if (currentRoomData && currentRoomData.id === roomId) {
    collectAllRoomMobisToInventory(false);
    await goToLobby();
  } else {
    (room.mobis || []).forEach(m => {
      if (m.type && m.type !== 'janela') setInventoryQty(m.type, getInventoryQty(m.type) + 1);
    });
    saveInventory();
  }

  const updated = all.filter(r => r.id !== roomId);
  onlineRooms = updated;
  localStorage.setItem('mw_rooms_cache', JSON.stringify(updated));
  if (currentUser?.uid) {
    await deleteDoc(doc(db, 'rooms', roomId)).catch(err => console.warn('Erro ao excluir quarto:', err));
  }
  renderRoomsModal();
}
...
function renderRoomsModal() {
  ...
  document.querySelectorAll('[data-delete-room-id]').forEach(btn => {
    btn.onclick = e => {
      e.stopPropagation();
      deleteRoom(btn.dataset.deleteRoomId);
    };
  });
...
function renderRoomCreateOptions() {
  const category = document.getElementById('newRoomCategory');
  if (category) category.innerHTML = roomCategories.map(c => `<option value="${escapeHtml(c.id)}">${escapeHtml(c.name)}</option>`).join('');
  const visitors = document.getElementById('newRoomMaxVisitors');
  if (visitors) visitors.innerHTML = roomVisitorOptions.map(v => `<option value="${v}">${v}</option>`).join('');
  const trade = document.getElementById('newRoomTradePreference');
  if (trade) trade.innerHTML = roomTradePreferences.map(t => `<option value="${escapeHtml(t.id)}">${escapeHtml(t.name)}</option>`).join('');
}
...
function renderShapeGrid() {
  ...
  roomShapes.forEach(shape => {
    ...
    btn.type = 'button';
    btn.className = 'shape-card habbo-shape-card ' + (selectedRoomShape.id === shape.id ? 'selected' : '');
    btn.innerHTML = `${roomShapePreviewHTML(shape)}<b>${shape.name}</b><small>${shape.squares || (shape.w * shape.d)} quadrados</small><em>${shape.desc}</em>`;
...
function createNewRoomFromModal() {
  ...
  const category = document.getElementById('newRoomCategory')?.value || 'bate-papo';
  const maxVisitors = Number(document.getElementById('newRoomMaxVisitors')?.value || 10);
  const tradePreference = document.getElementById('newRoomTradePreference')?.value || 'nao_permitidas';
  ...
  if (allMine.length >= 7) return alert('Você atingiu o limite máximo de 7 quartos. Exclua um quarto antes de criar outro.');
  const record = createRoomRecord({
    ...
    maxVisitors,
    tradePreference,
    baseRoom: selectedRoom,
    shape: selectedRoomShape,
    isFirst: allMine.length === 0
  });
  ['newRoomName','newRoomDescription','newRoomPassword'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.value = '';
  });
  renderRoomsModal();
...
async function startGame() {
  ...
  toggleAdminButton();
  refreshAvatarButton();
  setTimeout(() => toggleAdminButton(), 700);
  setTimeout(() => refreshAvatarButton(), 700);
...
function getRoomShapeSections(shape = getRoomShape(currentRoomData || { shapeId: 'quadrado' })) {
  return Array.isArray(shape.sections) && shape.sections.length
    ? shape.sections
    : [{ x: 0, z: 0, w: Number(shape.w || 14), d: Number(shape.d || 14) }];
}

function isPointOnRoomFloor(x, z, margin = 0.05) {
  return roomWalkSections.some(section => {
    const minX = Number(section.x || 0) - Number(section.w || 0) / 2 + margin;
    const maxX = Number(section.x || 0) + Number(section.w || 0) / 2 - margin;
    const minZ = Number(section.z || 0) - Number(section.d || 0) / 2 + margin;
    const maxZ = Number(section.z || 0) + Number(section.d || 0) / 2 - margin;
    return x >= minX && x <= maxX && z >= minZ && z <= maxZ;
  });
}

function nearestPointOnRoomFloor(x, z, margin = 0.05) {
  let best = null;
  let bestDist = Infinity;
  roomWalkSections.forEach(section => {
    const minX = Number(section.x || 0) - Number(section.w || 0) / 2 + margin;
    const maxX = Number(section.x || 0) + Number(section.w || 0) / 2 - margin;
    const minZ = Number(section.z || 0) - Number(section.d || 0) / 2 + margin;
    const maxZ = Number(section.z || 0) + Number(section.d || 0) / 2 - margin;
  ...
  return best || { x: roomSpawnPoint.x || 0, z: roomSpawnPoint.z || 0 };
}

function clampPointToRoomFloor(x, z, margin = 0.05) {
  const boundedX = Math.max(roomBounds.minX + margin, Math.min(roomBounds.maxX - margin, x));
  const boundedZ = Math.max(roomBounds.minZ + margin, Math.min(roomBounds.maxZ - margin, z));
  if (isPointOnRoomFloor(boundedX, boundedZ, margin)) return { x: boundedX, z: boundedZ };
  return nearestPointOnRoomFloor(boundedX, boundedZ, margin);
}

function createRoom(room) {
  room = {
    ...room,
    floor: Number(currentRoomData?.customFloorColor || room.floor),
    wall: Number(currentRoomData?.customWallColor || room.wall)
  };
  const shape = getRoomShape(currentRoomData || { shapeId: 'quadrado' });
  roomWalkSections = getRoomShapeSections(shape);
  const shell = createRoomShell(THREE, block, room, shape);
  roomBounds = shell.bounds;
  floorMesh = shell.floorMesh;
  roomSpawnPoint = clampPointToRoomFloor(shell.spawnPoint.x, shell.spawnPoint.z, 0.55);
  roomSpawnPoint.rot = shell.spawnPoint.rot || Math.PI;

  // Porta fixa: não é mobi e não pode ser recolhida ou ajustada.
  const doorX = shell.door.x;
  const doorZ = shell.door.z;
  ...
  const savedMobis = (currentRoomData && currentRoomData.mobis && currentRoomData.mobis.length)
    ...
    : [{ type: 'janela', x: 0, y: 0, z: roomBounds.minZ + 0.48, rot: 0, wallMobi: true }];
...
async function applyRoomPaint(item) {
  if (!item || !item.paintType) return false;
  if (!currentRoomData || !isCurrentRoomOwner()) {
    alert('Você só pode pintar o seu próprio quarto.');
    return true;
  }
  const price = Number(item.price || 50);
  if (Number(coins || 0) < price) {
  ...
  if (item.paintType === 'wall') {
    currentRoomData.customWallColor = Number(item.paintColor || item.color);
    update.customWallColor = currentRoomData.customWallColor;
  }
  if (item.paintType === 'floor') {
    currentRoomData.customFloorColor = Number(item.paintColor || item.color);
    update.customFloorColor = currentRoomData.customFloorColor;
  }

  const all = getStoredRooms();
  const idx = all.findIndex(r => r.id === currentRoomData.id);
  if (idx >= 0) {
    all[idx] = { ...all[idx], ...currentRoomData };
    onlineRooms = all;
    localStorage.setItem('mw_rooms_cache', JSON.stringify(all));
  }

  if (currentUser?.uid) {
    await setDoc(doc(db, 'rooms', currentRoomData.id), update, { merge: true }).catch(err => console.warn('Erro ao salvar tinta:', err));
  }
  ...
  // Recarrega o quarto para aplicar imediatamente no 3D.
  const keepPlayerPos = player ? { x: player.position.x, z: player.position.z, rot: player.rotation.y } : null;
  clearSceneForRoom();
  const light = new THREE.DirectionalLight(0xffffff, 1.1);
  light.position.set(5, 10, 5);
  light.castShadow = false;
  ...
  createRoom(getRoomTheme(currentRoomData));
  createPlayer();
  if (keepPlayerPos && player) {
    const safe = clampPointToRoomFloor(keepPlayerPos.x, keepPlayerPos.z, 0.48);
    player.position.x = safe.x;
    player.position.z = safe.z;
    player.rotation.y = keepPlayerPos.rot || 0;
  }
  updateCameraPosition();
  startRoomRealtime();
  addMessage('Sistema', `${item.paintType === 'wall' ? 'Parede' : 'Piso'} pintado com sucesso.`);
  return true;
}
...
async function buyMobi(type) {
  ...
  if (item.paintType) return applyRoomPaint(item);
  const price = Number(item.price || 50);
...
function startMobiPreview(type) {
  ...
  else { const p = clampPointToRoomFloor(player ? player.position.x + 1.6 : 0, player ? player.position.z : 2, 0.08); previewMobi.position.set(p.x, 0, p.z); }
  scene.add(previewMobi);
  showMobiControls('preview');
}


function getMobiCatalogItem(type) {
  return mobiCatalog.find(m => m.id === type) || null;
}

function isCambioMobi(type) {
  return !!getMobiCatalogItem(type)?.cambioValue;
}
...
function showMobiControls(mode) {
  ...
  const useBtn = document.getElementById('useMobiBtn');
  const selectedType = selectedPlacedMobi?.userData?.mobiType || '';
  if (useBtn) useBtn.classList.toggle('hidden', !(mode === 'placed' && isCambioMobi(selectedType)));
...
function useSelectedCambio() {
  if (!isCurrentRoomOwner()) return warnNotRoomOwner();
  if (!selectedPlacedMobi) return;
  const type = selectedPlacedMobi.userData?.mobiType;
  const item = getMobiCatalogItem(type);
  const value = Number(item?.cambioValue || 0);
  if (!value) return alert('Este mobi não é um câmbio.');

  if (!confirm(`Usar ${item.name} e receber ${value} moedas? O câmbio será removido do quarto.`)) return;

  coins = Number(coins || 0) + value;
  saveCoins();
  renderCoins();
  ...
  hideMobiControls();
  saveCurrentRoomMobis();
  addMessage('Sistema', `Você resgatou ${value} moedas.`);
}
...
function clampMobiInsideRoom(obj) {
  ...
  if (obj.userData && obj.userData.wallMobi) {
    const margin = 0.08;
    const wallBackDist = Math.abs(obj.position.z - (roomBounds.minZ + 0.48));
    const wallLeftDist = Math.abs(obj.position.x - (roomBounds.minX + 0.48));
    if (wallLeftDist < wallBackDist) {
      obj.position.x = roomBounds.minX + 0.48;
      obj.position.z = Math.max(roomBounds.minZ + 1.0, Math.min(roomBounds.maxZ - 1.0, obj.position.z));
      obj.rotation.y = Math.PI / 2;
    } else {
      obj.position.z = roomBounds.minZ + 0.48;
  ...
  const minX = roomBounds.minX + fp.halfX + margin;
  const maxX = roomBounds.maxX - fp.halfX - margin;
  const minZ = roomBounds.minZ + fp.halfZ + margin;
  const maxZ = roomBounds.maxZ - fp.halfZ - margin;

  obj.position.x = Math.max(minX, Math.min(maxX, obj.position.x));
  obj.position.z = Math.max(minZ, Math.min(maxZ, obj.position.z));

  const point = clampPointToRoomFloor(obj.position.x, obj.position.z, 0.08);
  obj.position.x = Math.max(minX, Math.min(maxX, point.x));
  obj.position.z = Math.max(minZ, Math.min(maxZ, point.z));
  obj.position.y = 0;
}
...
function moveActiveMobiTo(point) {
  ...
  if (obj.userData && obj.userData.wallMobi) {
    ...
    const backZ = roomBounds.minZ + 0.48;
    const leftX = roomBounds.minX + 0.48;
  ...
  const validPoint = clampPointToRoomFloor(point.x, point.z, 0.08);
  const fp = getMobiFootprint(obj);
  obj.position.x = Math.max(roomBounds.minX + fp.halfX + 0.02, Math.min(roomBounds.maxX - fp.halfX - 0.02, validPoint.x));
  obj.position.z = Math.max(roomBounds.minZ + fp.halfZ + 0.02, Math.min(roomBounds.maxZ - fp.halfZ - 0.02, validPoint.z));
  clampMobiInsideRoom(obj);
  if (selectedPlacedMobi && !previewMobi) saveCurrentRoomMobis();
  return true;
}
...
function setupMobiButtons() {
  ...
  document.getElementById('useMobiBtn')?.addEventListener('click', useSelectedCambio);
  document.getElementById('cancelMobiBtn').onclick = cancelMobiAction;
}

function createPlayer() {
  const base = currentUser.profile.character || selectedCharacter || characters[0];
  const c = { ...(characters.find(ch => ch.id === Number(base.id)) || characters[0]), ...base };
  currentUser.profile.character = c;

  const group = createAvatarModel(THREE, block, c, currentUser.profile, 'player');

  const rawSpawn = roomSpawnPoint || { x: 0, z: 0, rot: 0 };
  const spawn = clampPointToRoomFloor(rawSpawn.x, rawSpawn.z, 0.55);
  spawn.rot = rawSpawn.rot || 0;
...
function movePlayer(dt = 1/60) {
  ...
  if (dx || dz) {
    moveTarget = null;
    const next = clampPointToRoomFloor(player.position.x + dx, player.position.z + dz, 0.48);
    const moved = Math.hypot(next.x - player.position.x, next.z - player.position.z);
    ...
    if (dist < 0.08) {
      const finalPoint = clampPointToRoomFloor(moveTarget.x, moveTarget.z, 0.48);
      player.position.x = finalPoint.x;
      player.position.z = finalPoint.z;
      moveTarget = null;
      isWalking = false;
    } else {
      const next = clampPointToRoomFloor(
  ...
  const clamped = clampPointToRoomFloor(player.position.x, player.position.z, 0.48);
...
function animatePlayerWalk(dt = 1/60) {
  ...
  if (isWalking) {
    ...
    if (p.handL) p.handL.rotation.x = step * 0.30;
    if (p.handR) p.handR.rotation.x = opposite * 0.30;
    if (p.footL) p.footL.rotation.x = opposite * 0.20;
    if (p.footR) p.footR.rotation.x = step * 0.20;
    p.armL.position.y = 1.03 + Math.abs(opposite) * 0.03;
    p.armR.position.y = 1.03 + Math.abs(step) * 0.03;
    p.legL.position.y = 0.38 + Math.max(0, step) * 0.05;
    p.legR.position.y = 0.38 + Math.max(0, opposite) * 0.05;
  } else {
    // Volta suavemente para pose parada.
    player.position.y *= 0.72;
    ['armL', 'armR', 'legL', 'legR', 'handL', 'handR', 'footL', 'footR'].forEach(name => {
      if (p[name]) p[name].rotation.x *= 0.72;
    });
    const breathe = Math.sin(performance.now() * 0.003) * 0.012;
    if (p.chest) p.chest.position.y += ((1.10 + breathe) - p.chest.position.y) * 0.12;
    if (p.head) p.head.position.y += ((1.82 + breathe * 0.6) - p.head.position.y) * 0.12;
    p.armL.position.y += (1.03 - p.armL.position.y) * 0.25;
    p.armR.position.y += (1.03 - p.armR.position.y) * 0.25;
    p.legL.position.y += (0.38 - p.legL.position.y) * 0.25;
    p.legR.position.y += (0.38 - p.legR.position.y) * 0.25;
  }
}
...
function onClick(event) {
  ...
  if (point) {
    ...
    const target = clampPointToRoomFloor(point.x, point.z, 0.48);
...
function canTradeInCurrentRoom(otherUid = '') {
  const pref = currentRoomData?.tradePreference || 'nao_permitidas';
  if (pref === 'permitidas') return true;
  if (pref === 'nao_permitidas') return false;
  if (pref === 'somente_dono') {
    const owner = currentRoomData?.owner;
    return currentUser?.uid === owner || otherUid === owner;
  }
  return false;
}

function tradePreferenceMessage() {
  const pref = currentRoomData?.tradePreference || 'nao_permitidas';
  if (pref === 'somente_dono') return 'Neste quarto, trocas só são permitidas com o dono.';
  return 'Neste quarto, trocas não são permitidas.';
}

async function requestTrade() {
  if (!selectedRemotePlayer || !selectedRemotePlayer.uid) return alert('Jogador inválido para troca.');
  if (!canTradeInCurrentRoom(selectedRemotePlayer.uid)) return alert(tradePreferenceMessage());
  closePlayerActionMenu();
...
async function acceptTradeRequest() {
  ...
  if (!canTradeInCurrentRoom(pendingTradeRequest.requesterUid)) return alert(tradePreferenceMessage());
...
async function openTradeSession(other, id) {
  if (!canTradeInCurrentRoom(other.uid)) return alert(tradePreferenceMessage());
...
function toggleAdminButton() {
  ...
  const allowed = isAdminUser || OWNER_ADMIN_EMAILS.includes(email);
...
function clearAdminPanelFields() {
  ['adminCoinsEmail','adminCoinsAmount','adminMobiEmail','adminMobiQty','adminGlobalMessage'].forEach(id=>{
    const el=document.getElementById(id);
    if(el) el.value='';
  });
  const result=document.getElementById('adminPlayerResult');
...
function renderAdminOnlinePlayers() {
  ...
  const presenceRef = ref(rtdb, 'roomPresence');
  onValue(presenceRef, snap => {
    const roomsPresence = snap.val() || {};
    const found = [];

    Object.entries(roomsPresence).forEach(([roomId, players]) => {
      if (!players || typeof players !== 'object') return;

      Object.entries(players).forEach(([uid, data]) => {
        if (!data) return;
        if (uid === currentUser?.uid) return; // não mostra o próprio admin
        if (data.online === false) return;
        if (isPresenceStale(data)) return;

        const roomInfo = (getStoredRooms() || []).find(r => r.id === roomId);
        found.push({
          uid,
          roomId,
    ...
    adminOnlinePlayersCache = found;

    box.innerHTML = found.length ? found.map(p => `
      <div class="admin-online-row">
        <span>${escapeHtml(p.profile?.name || p.email || p.uid)} <small>• ${escapeHtml(p.roomName || p.roomId || 'quarto')}</small></span>
        <button type="button" data-admin-teleport="${escapeHtml(p.uid)}">Ir até</button>
      </div>
    `).join('') : 'Nenhum outro jogador online no jogo.';
...
function adminTeleportToPlayer(uid) {
  ...
  if (target.roomId && (!currentRoomData || currentRoomData.id !== target.roomId)) {
    const room = (getStoredRooms() || []).find(r => r.id === target.roomId);
    if (!room) return alert('Não consegui encontrar o quarto desse jogador.');
    enterRoom(room);
    setTimeout(() => {
      if (!player) return;
...
function collectAllRoomMobisToInventory(showAlert = true) {
  if (!currentRoomData || !isCurrentRoomOwner()) {
    if (showAlert) alert('Você só pode limpar seus próprios quartos.');
    return false;
  }
  const toCollect = placedMobis.filter(m => m.userData?.mobiType && m.userData.mobiType !== 'janela');
  if (!toCollect.length) {
    if (showAlert) alert('Não há mobis para recolher neste quarto.');
    return true;
  }
  toCollect.forEach(m => {
    const type = m.userData.mobiType;
    setInventoryQty(type, getInventoryQty(type) + 1);
    if (scene) scene.remove(m);
  });
  placedMobis = placedMobis.filter(m => !m.userData?.mobiType || m.userData.mobiType === 'janela');
  saveInventory();
  saveCurrentRoomMobis();
  renderInventory();
  if (showAlert) addMessage('Sistema', 'Todos os mobis do quarto foram recolhidos para o inventário.');
  return true;
}

function handleChatCommand(text) {
  const cmd = text.trim().toLowerCase();
  if (cmd === '/limpar quarto' || cmd === '/limparquarto') {
    collectAllRoomMobisToInventory(true);
    return true;
  }
  return false;
}
...
function refreshAvatarButton() {
  ...
  if (!btn) return;
  const inGame = !!currentUser && !!currentRoomData;
  btn.classList.toggle('hidden', !inGame);
}

function fillAvatarSelect(id, group, selectedId) {
  const select = document.getElementById(id);
  if (!select) return;
  const items = avatarItems[group] || [];
  select.innerHTML = items.map(item => `<option value="${escapeHtml(item.id)}">${escapeHtml(item.name)}</option>`).join('');
  select.value = selectedId || items[0]?.id || '';
}

function getAvatarDraftFromEditor(base = {}) {
  return {
    ...base,
    skinId: document.getElementById('avatarSkinItem')?.value || base.skinId || 'pele_clara',
    hairId: document.getElementById('avatarHairItem')?.value || base.hairId || 'curto_preto',
    faceId: document.getElementById('avatarFaceItem')?.value || base.faceId || 'normal',
    shirtId: document.getElementById('avatarShirtItem')?.value || base.shirtId || 'camisa_azul',
    pantsId: document.getElementById('avatarPantsItem')?.value || base.pantsId || 'calca_jeans',
    shoesId: document.getElementById('avatarShoesItem')?.value || base.shoesId || 'tenis_preto',
    accessoryId: document.getElementById('avatarAccessoryItem')?.value || base.accessoryId || 'none',
    wingsId: document.getElementById('avatarWingsItem')?.value || base.wingsId || 'none'
  };
}

function updateAvatarMiniPreview() {
  const root = document.getElementById('avatarPreview');
  if (!root) return;
  const base = currentUser?.profile?.character || selectedCharacter || {};
  const c = applyAvatarItemIds(getAvatarDraftFromEditor(base));

  root.style.setProperty('--skin', c.skin || '#f2b879');
  root.style.setProperty('--hair', c.hair || '#020617');
  root.style.setProperty('--shirt', c.shirt || '#2563eb');
  root.style.setProperty('--pants', c.pants || '#1e3a8a');
  root.style.setProperty('--shoes', c.shoes || '#111827');
  root.style.setProperty('--accessory', c.accessoryColor || '#111827');
  root.style.setProperty('--wings', c.wingsColor || '#f8fafc');

  root.dataset.hairStyle = c.hairStyle || 'curto';
  root.dataset.eyeStyle = c.eyeStyle || 'normal';
  root.dataset.mouthStyle = c.mouthStyle || 'sorriso';
  root.dataset.accessory = c.accessoryType || 'none';
  root.dataset.wings = c.wingsType || 'none';
}
...
function setupAvatarEditor() {
  ...
  btn.onclick = () => {
    const c = currentUser?.profile?.character || selectedCharacter || {};
    fillAvatarSelect('avatarSkinItem', 'skins', c.skinId || 'pele_clara');
    fillAvatarSelect('avatarHairItem', 'hairs', c.hairId || 'curto_preto');
    fillAvatarSelect('avatarFaceItem', 'faces', c.faceId || 'normal');
    fillAvatarSelect('avatarShirtItem', 'shirts', c.shirtId || 'camisa_azul');
    fillAvatarSelect('avatarPantsItem', 'pants', c.pantsId || 'calca_jeans');
    fillAvatarSelect('avatarShoesItem', 'shoes', c.shoesId || 'tenis_preto');
    fillAvatarSelect('avatarAccessoryItem', 'accessories', c.accessoryId || 'none');
    fillAvatarSelect('avatarWingsItem', 'wings', c.wingsId || 'none');

    ['avatarSkinItem','avatarHairItem','avatarFaceItem','avatarShirtItem','avatarPantsItem','avatarShoesItem','avatarAccessoryItem','avatarWingsItem'].forEach(id => {
      const el = document.getElementById(id);
      if (el) el.onchange = updateAvatarMiniPreview;
    });

    updateAvatarMiniPreview();
    modal.classList.remove('hidden');
  };

  document.getElementById('closeAvatarBtn').onclick = () => {
    modal.classList.add('hidden');
  };

  document.getElementById('saveAvatarBtn').onclick = async () => {
    if (!currentUser?.profile?.character) return;
    const updated = applyAvatarItemIds(getAvatarDraftFromEditor(currentUser.profile.character));
    currentUser.profile.character = updated;

    if (currentUser.uid) {
      await setDoc(doc(db, 'users', currentUser.uid), {
        profile: { ...(currentUser.profile || {}), character: updated, characterId: updated.id },
        updatedAt: serverTimestamp()
      }, { merge: true }).catch(err => console.warn('Erro ao salvar avatar:', err));
    }

    modal.classList.add('hidden');

    if (player && scene) {
      const oldPos = { x: player.position.x, z: player.position.z, rot: player.rotation.y };
      scene.remove(player);
      player = null;
      createPlayer();
      if (player) {
...
document.addEventListener('visibilitychange', () => {
  if (!document.hidden) { syncMyPresence(true); keepGameFullscreen(); }
});
