...
const REMOTE_POSITION_LERP = 0.22;
const REMOTE_ROTATION_LERP = 0.24;
const REMOTE_TELEPORT_DISTANCE = 4.5;
const PRESENCE_MOVE_THRESHOLD = 0.10;
const PRESENCE_ROT_THRESHOLD = 0.10;
const PRESENCE_SEND_INTERVAL_MS = 120;
...
function updateRemotePlayer(uid, data) {
  ...
  if (!rp) {
    ...
    rp = {
      ...
      target,
      lastSpeechId: null,
      lastVisualX: target.x,
      lastVisualZ: target.z,
...
function updateRemotePlayers(dt = 1/60) {
  remotePlayers.forEach(rp => {
    ...
    if (distance > REMOTE_TELEPORT_DISTANCE) {
      ...
      // Lerp compensado por delta para manter suavidade parecida em 30/60/120 FPS.
    ...
    g.rotation.y += shortestAngleDelta(g.rotation.y, t.rot) * rotAlpha;

    const movedVisually = Math.hypot(g.position.x - (rp.lastVisualX ?? g.position.x), g.position.z - (rp.lastVisualZ ?? g.position.z)) > 0.003 || distance > 0.04;
    rp.lastVisualX = g.position.x;
    rp.lastVisualZ = g.position.z;
    animateRemoteWalk(g, movedVisually);
  });
}
