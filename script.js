...
function updateRemotePlayer(uid, data) {
  ...
  if (!rp) {
    const group = makeRemotePlayerGroup(data.profile);
  ...
  g.userData.profile = { ...data.profile, character: characters.find(c => c.id === Number(data.profile.characterId)) || characters[0] };
...
function onClick(event) {
  ...
  const hitPlayer = hits.find(h => findRootByType(h.object, 'player'));
  if (hitPlayer && !previewMobi && !mobiMoveMode) {
    openProfile(currentUser.profile);
    return;
  }
