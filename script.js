...
function startPresenceHeartbeat() {
  ...
  presenceHeartbeatTimer = setInterval(() => syncMyPresence(true), 25000);
}
...
function syncMyPresence(forceOrSpeech = false) {
  ...
  if (!speechText) {
    ...
    const movedEnough = !hasSnapshot || Math.hypot(x - lastPresenceSnapshot.x, z - lastPresenceSnapshot.z) >= 0.28;
    const turnedEnough = !hasSnapshot || Math.abs(rot - lastPresenceSnapshot.rot) >= 0.35;

    if (isForcedHeartbeat) {
      if (now - lastPresenceSync < 20000 && hasSnapshot) return;
    } else {
      if (!movedEnough && !turnedEnough) return;
      if (now - lastPresenceSync < 1000) return;
    }
  }

  const state = JSON.stringify({ x, z, rot, room: currentRoomData.id, speech: speechText });
  if (!speechText && state === lastPresenceState && now - lastPresenceSync < 20000) return;
  ...
  setDoc(doc(db, 'rooms', currentRoomData.id, 'players', currentUser.uid), {
    ...
    position: { x, y: Number(player.position.y.toFixed(3)), z, rot },
