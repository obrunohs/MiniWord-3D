...
function createRoomRecord({ name, pass = '', description = '', category = 'bate-papo', maxVisitors = 10, tradePreference = 'nao_permitidas', baseRoom = selectedRoom, shape = selectedRoomShape, isFirst = false }) {
  ...
  const record = {
    ...
    shapeId: shape.id,
    createdAt: Date.now(),
    mobis: [ { type: 'janela', x: 0, y: 0, z: -(shape.d / 2) + 0.32, rot: 0, wallMobi: true } ],
    isFirst
  };
  all.push(record);
  saveStoredRooms(all);
...
function createRoom(room) {
  ...
  const savedMobis = (currentRoomData && currentRoomData.mobis && currentRoomData.mobis.length)
    ...
    : [{ type: 'janela', x: 0, y: 0, z: roomBounds.minZ + 0.32, rot: 0, wallMobi: true }];
...
function startMobiPreview(type) {
  ...
  else { const p = clampPointToRoomFloor(player ? player.position.x + 1.6 : 0, player ? player.position.z : 2, 0.35); previewMobi.position.set(p.x, 0, p.z); }
...
function clampMobiInsideRoom(obj) {
  ...
  const margin = 0.02;
  const point = clampPointToRoomFloor(obj.position.x, obj.position.z, Math.max(fp.halfX, fp.halfZ) + margin);
...
function moveActiveMobiTo(point) {
  ...
  const validPoint = clampPointToRoomFloor(point.x, point.z, 0.3);
