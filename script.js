...
const PRESENCE_SEND_INTERVAL_MS = 85;
...
function syncMyPresence(forceOrSpeech = false) {
  ...
  // Proteção de cota do Firebase:
  // antes o jogo podia gravar presença várias vezes por segundo enquanto o jogador andava.
  // Agora só grava quando mudou bastante, ou em heartbeat lento, ou quando envia fala.
  if (!speechText) {
...
async function completeTrade(data) {
  ...
  // Permite trade unilateral: basta pelo menos um dos dois oferecer algum mobi.
  if (!offerHasItems(myOffer) && !offerHasItems(otherOffer)) {
    return alert('Escolha pelo menos um mobi para concluir o trade.');
  }

  // Antes de concluir, valida apenas o inventário de quem está executando a conclusão.
  // O outro lado também valida/aplica a própria parte quando receber o snapshot.
  for (const [type, qty] of Object.entries(myOffer.items)) {
    if (getInventoryQty(type) < qty) return alert(`Você não tem ${qty} unidade(s) de ${mobiName(type)}.`);
  }
  ...
  const delivery = {};
  delivery[myUid] = {
    remove: myOffer.items,
    add: otherOffer.items
  };
  delivery[otherUid] = {
    remove: otherOffer.items,
    add: myOffer.items
  };
...
async function applyCompletedTrade(data) {
  ...
  const myUid = currentUser.uid;
  const tradeKey = `mw_trade_applied_${currentTrade.id}_${myUid}`;
  if (data.applied?.[myUid] || localStorage.getItem(tradeKey) === '1') return;

  let myDelivery = data.delivery?.[myUid];

  // Compatibilidade com trades antigos que não tinham delivery.
  if (!myDelivery) {
    const players = Array.isArray(data.players) ? data.players : [];
    const otherUid = players.find(uid => uid !== myUid) || currentTrade.other.uid;
    const myOffer = normalizeOffer(data.offers?.[myUid]);
    const otherOffer = normalizeOffer(data.offers?.[otherUid]);
    myDelivery = { remove: myOffer.items, add: otherOffer.items };
  }

  const toRemove = myDelivery.remove || {};
  const toAdd = myDelivery.add || {};

  for (const [type, qty] of Object.entries(toRemove)) {
    if (getInventoryQty(type) < Number(qty || 0)) {
      document.getElementById('tradeStatus').textContent = `Troca não aplicada: faltam unidades de ${mobiName(type)}.`;
      return;
    }
  }

  Object.entries(toRemove).forEach(([type, qty]) => {
    setInventoryQty(type, getInventoryQty(type) - Number(qty || 0));
  });

  Object.entries(toAdd).forEach(([type, qty]) => {
    setInventoryQty(type, getInventoryQty(type) + Number(qty || 0));
  });

  localStorage.setItem(tradeKey, '1');
  saveInventory();
  renderInventory();
  renderTradeInventory();
  ...
  await setDoc(doc(db, 'trades', currentTrade.id), {
    applied: { [myUid]: true },
    updatedAt: serverTimestamp()
  }, { merge: true }).catch(err => console.warn('Erro ao marcar trade aplicado:', err));
