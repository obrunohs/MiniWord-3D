...
function isPresenceStale(data) {
  ...
  return !lastSeen || Date.now() - lastSeen > 12000;
}
...
async function openTradeSession(other, id) {
  ...
  if (!snap.exists() || snap.data().completed || snap.data().roomId !== currentRoomData.id) {
    await setDoc(ref, {
      id,
      roomId: currentRoomData.id,
...
function renderTradeInventory() {
  ...
  if (!wrap) return;
  wrap.innerHTML = inventory.map(item => `<button class="trade-item" data-trade-mobi="${item.id}" ${item.qty <= 0 ? 'disabled' : ''}>${mobiPreviewHTML(item.id)}<b>${escapeHtml(item.name)}</b><em>x${item.qty}</em></button>`).join('');
  wrap.querySelectorAll('[data-trade-mobi]').forEach(btn => {
    btn.onclick = () => setTradeOffer(btn.dataset.tradeMobi);
  });
}

async function setTradeOffer(type) {
  if (!currentTrade) return;
  if (getInventoryQty(type) <= 0) return alert('Você não tem esse mobi no inventário.');
  await setDoc(doc(db, 'trades', currentTrade.id), {
    offers: { [currentUser.uid]: type },
    confirms: { [currentUser.uid]: false },
    updatedAt: serverTimestamp()
  }, { merge: true });
}
...
function renderTradeState(data) {
  ...
  const myOffer = data.offers?.[currentUser.uid] || '';
  const otherOffer = data.offers?.[otherUid] || '';
  const myConfirm = !!data.confirms?.[currentUser.uid];
  const otherConfirm = !!data.confirms?.[otherUid];
  document.getElementById('myTradeOffer').innerHTML = myOffer ? mobiPreviewHTML(myOffer) + `<b>${mobiName(myOffer)}</b>` : '<span>Nenhum mobi escolhido</span>';
  document.getElementById('otherTradeOffer').innerHTML = otherOffer ? mobiPreviewHTML(otherOffer) + `<b>${mobiName(otherOffer)}</b>` : '<span>Aguardando outro jogador</span>';
  ...
  if (data.completed) {
    ...
    applyCompletedTrade(data).catch(err => console.warn('Erro ao aplicar troca:', err));
    return;
  }
  if (!data.completed && myOffer && otherOffer && myConfirm && otherConfirm) completeTrade(data).catch(err => alert('Erro ao concluir troca: ' + err.message));
}
...
async function confirmTrade() {
  ...
  const data = snap.data() || {};
  const myOffer = data.offers?.[currentUser.uid];
  if (!myOffer) return alert('Escolha um mobi para oferecer primeiro.');
...
async function completeTrade(data) {
  if (!currentTrade || data.completed) return;
  const myUid = currentUser.uid;
  const otherUid = currentTrade.other.uid;
  const myOffer = data.offers?.[myUid];
  const otherOffer = data.offers?.[otherUid];
  if (!myOffer || !otherOffer) return;
  await setDoc(doc(db, 'trades', currentTrade.id), { completed: true, completedAt: serverTimestamp() }, { merge: true });
}
...
async function applyCompletedTrade(data) {
  ...
  const myOffer = data.offers?.[myUid];
  const otherOffer = data.offers?.[otherUid];
  if (!myOffer || !otherOffer || getInventoryQty(myOffer) <= 0) return;
  setInventoryQty(myOffer, getInventoryQty(myOffer) - 1);
  setInventoryQty(otherOffer, getInventoryQty(otherOffer) + 1);
  saveInventory();
  renderInventory();
  renderTradeInventory();
...
window.addEventListener('pagehide', () => { removeMyPresence(); });
document.addEventListener('visibilitychange', () => {
  if (document.hidden) removeMyPresence();
  else syncMyPresence(true);
});
