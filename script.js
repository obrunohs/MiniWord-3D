...
function roomListItem(record, mine = false) {
  ...
  return `<div class="room-list-item"><div><b>${escapeHtml(record.name)}${lock}</b><small>Dono: ${escapeHtml(record.ownerName || record.owner)} • ${shape.name} • ${theme.name} • ${escapeHtml(record.category || 'bate-papo')} • máx. ${Number(record.maxVisitors || 10)}</small></div><button data-room-id="${record.id}" type="button">Ir ao quarto</button></div>`;
}
...
function createRoom(room) {
  ...
  const savedMobis = (currentRoomData && currentRoomData.mobis && currentRoomData.mobis.length)
    ...
    : [{ type: 'janela', x: 0, y: 0, z: roomBounds.minZ + 0.18, rot: 0, wallMobi: true }];
