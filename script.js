...
function packedInventoryToMap(packed) {
  const map = {};
  mobiCatalog.forEach(m => map[m.id] = 0);
  (Array.isArray(packed) ? packed : []).forEach(item => {
    if (item && item.id) map[item.id] = Number(item.qty) || 0;
  });
  return map;
}

function inventoryMapToPacked(map) {
  return mobiCatalog.map(m => ({ id: m.id, qty: Math.max(0, Number(map[m.id]) || 0) }));
}

function addOfferToInventoryMap(map, offer, multiplier = 1) {
  const normalized = normalizeOffer(offer);
  Object.entries(normalized.items).forEach(([type, qty]) => {
    map[type] = Math.max(0, (Number(map[type]) || 0) + (Number(qty) || 0) * multiplier);
  });
}
...
async function reloadInventoryFromCloud() {
  ...
  const packed = userSnap.data()?.inventory || [];
  inventory = mobiCatalog.map(m => ({ ...m, qty: (packed.find(x => x.id === m.id) || {}).qty || 0 }));
  localStorage.setItem(getUserInventoryKey(), JSON.stringify(inventory.map(i => ({ id: i.id, qty: i.qty }))));
  renderInventory();
...
async function completeTrade(data) {
  ...
  await runTransaction(db, async (transaction) => {
    const freshTradeSnap = await transaction.get(tradeRef);
    if (!freshTradeSnap.exists()) throw new Error('Trade não encontrado.');

    const freshTrade = freshTradeSnap.data() || {};
    if (freshTrade.completed || freshTrade.status === 'completed') return;
    if (freshTrade.status === 'canceled') throw new Error('Trade cancelado.');

    // v15.3: a sessão de trade salva os participantes no campo `players`.
    // A versão anterior tentava ler `participants`, então o status virava
    // concluído em alguns casos, mas o inventário não era transferido.
    const users = Array.isArray(freshTrade.players)
      ? freshTrade.players.filter(Boolean)
      : Object.keys(freshTrade.participants || {});
    const uniqueUsers = [...new Set(users)];
    if (uniqueUsers.length !== 2) throw new Error('Trade inválido.');

    const [uidA, uidB] = uniqueUsers;
    const offerA = normalizeOffer(freshTrade.offers?.[uidA]);
    const offerB = normalizeOffer(freshTrade.offers?.[uidB]);
    if (!offerHasItems(offerA) && !offerHasItems(offerB)) throw new Error('Adicione pelo menos um mobi para concluir.');

    if (!freshTrade.confirms?.[uidA] || !freshTrade.confirms?.[uidB]) return;
    ...
    const invA = packedInventoryToMap(userASnap.data()?.inventory || []);
    const invB = packedInventoryToMap(userBSnap.data()?.inventory || []);

    for (const [type, qty] of Object.entries(offerA.items)) {
      if ((Number(invA[type]) || 0) < qty) throw new Error(`Um dos jogadores não tem unidades suficientes de ${mobiName(type)}.`);
    }
    for (const [type, qty] of Object.entries(offerB.items)) {
      if ((Number(invB[type]) || 0) < qty) throw new Error(`Um dos jogadores não tem unidades suficientes de ${mobiName(type)}.`);
    }

    addOfferToInventoryMap(invA, offerA, -1);
    addOfferToInventoryMap(invB, offerA, 1);
    addOfferToInventoryMap(invB, offerB, -1);
    addOfferToInventoryMap(invA, offerB, 1);

    transaction.set(userARef, { inventory: inventoryMapToPacked(invA), updatedAt: serverTimestamp() }, { merge: true });
    transaction.set(userBRef, { inventory: inventoryMapToPacked(invB), updatedAt: serverTimestamp() }, { merge: true });
    transaction.set(tradeRef, { completed: true, status: 'completed', completedAt: serverTimestamp(), updatedAt: serverTimestamp() }, { merge: true });
  });
}
