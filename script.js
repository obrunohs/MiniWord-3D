...
function movePlayer(dt = 1/60) {
  ...
  if (dx || dz) {
    moveTarget = null;
    const next = clampPointToRoomFloor(player.position.x + dx, player.position.z + dz, 0.08);
    ...
    if (dist < 0.08) {
      const finalPoint = clampPointToRoomFloor(moveTarget.x, moveTarget.z, 0.08);
  ...
  const clamped = clampPointToRoomFloor(player.position.x, player.position.z, 0.08);
...
function onClick(event) {
  ...
  if (point) {
    ...
    const target = clampPointToRoomFloor(point.x, point.z, 0.08);
